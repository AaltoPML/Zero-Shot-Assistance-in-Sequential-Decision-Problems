# This file collects the data from all the runs of the day trip design experiment, post-processes it, and generates the plots used in the paper.
#
#
...
AIAD_NMA_utilities = Array{Array{Float64,1},1}()
AIAD_NMA_posterior_entropies = Array{Array{Float64,1},1}()
for i in EXP_IDXS
    @load "$(DATADIR)/$(EXP_NAME)_AIAD_NMA_$(i).jld" utility_history posterior_history
    posterior_entropy_history = [entropy2(posterior_history[i,:]) for i in 1:size(posterior_history,1)]

    push!(AIAD_NMA_utilities, utility_history)
    push!(AIAD_NMA_posterior_entropies, posterior_entropy_history)
end
...
@load "$(DATADIR)/$(EXP_NAME)_anchoring_experiment_not_modeled_assumed.jld" user_models true_user_model

AIAD_NMA_expected_reward_error = []
for exp_IDX in EXP_IDXS
    @load "$(DATADIR)/$(EXP_NAME)_AIAD_NMA_$(exp_IDX).jld" state_history utility_history posterior_history
    ...
    push!(AIAD_NMA_expected_reward_error, E_distance)
end
...
@save "experiment_data_anchoring_experiment.jld" TRUE_optimized AIAD_utilities AIAD_expected_reward_error AIAD_posterior_entropies AIAD_automation_utilities AIAD_automation_posterior_entropies AIAD_automation_expected_reward_error AIAD_automation_tot_num_interactions AIAD_NM_utilities AIAD_NM_posterior_entropies AIAD_NM_expected_reward_error AIAD_NMA_utilities AIAD_NMA_posterior_entropies AIAD_NMA_expected_reward_error IRL_utilities IRL_posterior_entropies IRL_expected_reward_error IRL_optimized_0_restart IRL_optimized_5_restart IRL_optimized_10_restart IRL_optimized_15_restart IRL_optimized_20_restart IRL_optimized_25_restart IRL_optimized_30_restart PL_posterior_entropies PL_expected_reward_error PL_optimized_5_restart PL_optimized_10_restart PL_optimized_15_restart PL_optimized_20_restart PL_optimized_25_restart PL_optimized_30_restart
...
savefig("trip_design_anchoring_experiment_utility_per_interaction.pdf")

#
# ablation
#
p = plot(legend = :bottomright, legend_column = 1, xlabel = "number of agent interactions", ylabel = "objective value", grid = true, gridalpha = 0.07, ylims = (0.0, Inf), size = (450,270))
plot!(p, collect(0:RUN_LENGTH), agg(AIAD_utilities, mean, RUN_LENGTH), ribbon = agg(AIAD_utilities, std, RUN_LENGTH) ./ sqrt(length(AIAD_utilities)) .* stds, fillalpha = alpha, color = palette(colorstyle)[1], label = "AIAD")
plot!(p, collect(0:RUN_LENGTH), agg(AIAD_NM_utilities, mean, RUN_LENGTH), ribbon = agg(AIAD_NM_utilities, std, RUN_LENGTH) ./ sqrt(length(AIAD_NM_utilities)) .* stds, fillalpha = alpha, color = palette(colorstyle)[2], label = "AIAD, no bias modeled")
plot!(p, collect(0:RUN_LENGTH), agg(AIAD_NMA_utilities, mean, RUN_LENGTH), ribbon = agg(AIAD_NMA_utilities, std, RUN_LENGTH) ./ sqrt(length(AIAD_NMA_utilities)) .* stds, fillalpha = alpha, color = palette(colorstyle)[3], label = "AIAD, anchoring bias assumed")
savefig("trip_design_anchoring_experiment_ablation.pdf")
...
savefig(p, "trip_design_anchoring_experiment_posterior_entropy_per_interaction.pdf")
...
savefig(p, "trip_design_anchoring_experiment_posterior_loss_per_interaction.pdf")
...
savefig(p, "trip_design_anchoring_experiment_cumulative_reward.pdf")
...
error_plot = plot(collect(0:RUN_LENGTH-1), mean(hcat(AIAD_expected_reward_error...), dims = 2)[1:RUN_LENGTH], color = palette(colorstyle)[1], label = "AIAD", ylabel = "error in ω", xlims = (0, Inf), legend = :topright)
plot!(error_plot, collect(0:RUN_LENGTH-1), mean(hcat(AIAD_automation_expected_reward_error...), dims = 2)[1:RUN_LENGTH], color = palette(colorstyle)[5], label = "AIAD + automation")
plot!(error_plot, [-1], [2.5], color = palette(colorstyle)[2], label = "random advice")

probs_plot = plot(legend_column = 1, xlabel = "number of agent interactions", ylabel = "acceptance rate", grid = true, gridalpha = 0.07, xlims = (0, 29), ylims = (0.0, Inf), legend = :none)
plot!(probs_plot, collect(0:RUN_LENGTH-1), AIAD_acceptance_rate[1:RUN_LENGTH], label = "AIAD", color = palette(colorstyle)[1])
plot!(probs_plot, collect(0:RUN_LENGTH-1), AIAD_automation_acceptance_rate[1:RUN_LENGTH], label = "AIAD + automation", color = palette(colorstyle)[5])
plot!(probs_plot, collect(0:RUN_LENGTH-1), mean(hcat(uniform_advice_acceptance_probability...), dims = 2)[1:RUN_LENGTH], label = "random advice", color = palette(colorstyle)[2])

p = plot(error_plot, probs_plot, layout = (2, 1))
savefig(p, "anchoring_experiment_acceptance_rate_analysis.pdf")
