...
mutable struct RDSAMDP{UMP <: Pair{WM,UM} where WM <: WorldModelSpecification where UM <: MCTSUserModelSpecification} <: RandomMDP{DaytripAndInfo, Union{DaytripEdit,EditRecommendation}}
    underlying_MDP::LockstepDaytripScheduleAssistanceMDP
    ...
    function RDSAMDP(user_models::Array{Pair{WM,UM},1}, weights::Array{Float64,1}, home::POI, POIs::Array{POI,1}, discounting::Float64, subsample_size::Union{Int64,Missing} = missing; cache_maxsize = 100000) where WM <: WorldModelSpecification where UM <:MCTSUserModelSpecification
        ...
        ret = new{Pair{WM,UM}}(LockstepDaytripScheduleAssistanceMDP(instantiate(user_models[idx], home, POIs), discounting), idx, user_models, weights, subsample_size, zeros(Float64, length(user_models)), home, POIs, discounting, :CW, LRU{Tuple{Int64,BigInt},Array{Float64,1}}(maxsize = cache_maxsize))
        ret.underlying_MDP = LockstepDaytripScheduleAssistanceMDP(instantiate(user_models[idx], home, POIs, ret), discounting)
...
function Random.rand!(m::RDSAMDP)
    ...
    m.underlying_MDP = LockstepDaytripScheduleAssistanceMDP(instantiate(m.user_models[idx], m.home, m.POIs, m), m.discounting)
...
function run_rootsampled_planning(um, user_models; estimate_value = 0.0, N = 10, n_iterations = 1000, planning_depth = 10, planning_subset_size = 100, max_time::Float64 = Inf)
    ...
    true_assMDP = LockstepDaytripScheduleAssistanceMDP(um, 1.0)
