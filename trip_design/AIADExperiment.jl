# This file implements the main loop for runnin AIAD. It supports both AIAD ad AIAD + automation, depending on the options set.
#
#
...
mutable struct RDSAMDP{UMP <: Pair{WM,UM} where WM <: WorldModelSpecification where UM <: MCTSUserModelSpecification} <: RandomMDP{DaytripAndInfo, Union{DaytripEdit,EditRecommendation}}
    ...
    function RDSAMDP(user_models::Array{Pair{WM,UM},1}, weights::Array{Float64,1}, home::POI, POIs::Array{POI,1}, discounting::Float64, subsample_size::Union{Int64,Missing} = missing; cache_maxsize = 100000, enable_direct_edits = false) where WM <: WorldModelSpecification where UM <:MCTSUserModelSpecification
        ...
        ret = new{Pair{WM,UM}}(DaytripScheduleAssistanceMDP(instantiate(user_models[idx], home, POIs), discounting), idx, user_models, weights, subsample_size, zeros(Float64, length(user_models)), home, POIs, enable_direct_edits, discounting, :CW, LRU{Tuple{Int64,BigInt},Array{Float64,1}}(maxsize = cache_maxsize))
        ret.underlying_MDP = DaytripScheduleAssistanceMDP(instantiate(user_models[idx], home, POIs, ret), discounting; enable_direct_edits = enable_direct_edits)
...
function Random.rand!(m::RDSAMDP)
    ...
    m.underlying_MDP = DaytripScheduleAssistanceMDP(instantiate(m.user_models[idx], m.home, m.POIs, m), m.discounting; enable_direct_edits = m.enable_direct_edits)
...
function run_rootsampled_planning(um, user_models; estimate_value = 0.0, N = 10, n_iterations = 1000, planning_depth = 10, planning_subset_size = 100, enable_direct_edits = false, max_time::Float64 = Inf)
    N_um = length(user_models)

     # This is the assistance problem with the true user model
    true_assMDP = DaytripScheduleAssistanceMDP(um,
                                               1.0;
                                               enable_direct_edits = enable_direct_edits)
...
function main()
    ...
    if length(Base.ARGS) in [3,4]
        ...
        enable_direct_edits = ( (length(Base.ARGS) == 4) && (Base.ARGS[4] == "AUTOMATE") )
        N = enable_direct_edits ? N*2 : N
        @show N
