...
export FINEARTS, POPCULTURE, SIGHTSEEING, HISTORICHOUSE, SHOPPING, HOME, POI, Daytrip, distance, travel_dist, travel_time, duration, cost, optimize_trip, optimize_trip_cw
...
"""
function get_edit_policy(um::CachedBoltzmannUserModel{CacheType}, s::Daytrip, a_recommended::Union{EditRecommendation,Missing}) where CacheType where WorldModel
...
mutable struct DaytripScheduleAssistanceMDP <: MDP{DaytripAndInfo, EditRecommendation}
    ...
    DaytripScheduleAssistanceMDP(m::MCTSUserModel, discounting::Float64; optimizer = :CW) = new(m, MachineDaytripScheduleMDP(m.world_model, discounting, optimizer = optimizer), m.world_model.POIs, m.world_model.home, discounting, optimizer)
end
...
function POMDPs.actions(m::DaytripScheduleAssistanceMDP, s::DaytripAndInfo)
    ...
    push!(ret, [EditRecommendation(e) for e in actions(m.objective_world_model, s.trip)]...)
    return ret
end
