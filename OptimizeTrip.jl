...
mutable struct RMDSMDP{UMP <: Pair{WM,UM} where WM <:WorldModelSpecification where UM <:MCTSUserModelSpecification} <: RandomMDP{Daytrip, DaytripEdit}
...
using Random
function Random.rand!(m::RMDSMDP)
...
function update_weights!(m::RMDSMDP, weights::Array{Float64,1})
    m.weights = weights
    return m
end
